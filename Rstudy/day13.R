...
library(xlsx)
classDF <- read.xlsx("data/data.xlsx", 1, encoding="UTF-8") #number는 시트번호
str(classDF)
View(classDF)
...
ggplot(coSalesDF, aes(x=year, y=sales)) + geom_line(size=2, aes(group=company, colour=company)) + geom_point(size=2, shape = '가') #shape를 한글로 줘도 된다. 
...
ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), colour="blue", stat="identity") + 
  coord_cartesian(ylim=c(60, 75))

ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + 
  geom_label(aes(label=weight))+coord_cartesian(ylim=c(60, 75)) # bar의 label을 출력. 

ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + 
  geom_label(aes(label=weight), nudge_y=1)+coord_cartesian(ylim=c(60, 75)) #nudge_y -> 떨어진 위치

ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + 
  geom_label(aes(label=weight), nudge_y=2)+coord_cartesian(ylim=c(60, 75))

ggplot(data=w, aes(x=year, y=weight)) + geom_bar(aes(fill=year), stat="identity") + 
  geom_label(aes(label=weight), nudge_y=-1)+coord_cartesian(ylim=c(60, 75)) + labs(title = "테스트", subtitle="ggplot2 패키지를 이용한 시각화", x="년도", y="무게")
...
#행설정
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer))+geom_point() +
  geom_point() + guides(color= guide_legend(nrow=10))

#열설정
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer))+geom_point() +
  geom_point() + scale_fill_brewer(palette="Reds")+ guide(color=guide_legend (ncol=2))

#범례 항목들 역순으로
ggplot(mpg, aes(x=displ, y=hwy, color=manufacturer)) +
  geom_point() + guides(color = guide_legend(reverse=TRUE)) #legend 출력을 역순으로
